...
New in this version: 
    Main change: When a new device is onboarded, it is done in an enabled state (earlier it was onboarded disabled). This simplifies
        many problems in discovery and initial in-memory buildups.
    A new API call is added: /list/room/stats
    This will list all rooms, their types and the device count (actually, relsen count) in each room        
    A few hard-coded room types and device types are added out of the box. The user can add more if he wants.
    A bug with issuing socket commands has been fixed: if a new device is not found in the in-memory structures, it threw an exception and failed; now
        all new devices are onboarded in an enabled state, so that they can be immediately added to the in-memory data structures.
    TODO: implement the actual wifi based Tasmota discovery and subsequently onboard them automatically.
