...
#   so that we can configure it and (2) Relsens of the new device are not yet configured, so most search operations will fail.
...
def enable_device (device_id):
    ...
    dev.enabled = True
    db.session.commit()   
    return ({'result' : 'successfully enabled device'})
...
def disable_device (device_id):
    ...
    dev.enabled = False
    db.session.commit()   
    return ({'result' : 'successfully disabled device'})    
...
def update_relsen (jrelsen):
    ...
    db.session.commit()    
    enable_device (device_id) # every time you configure a device, mark it enabled; this is useful for new devices that are onboarded in a disabled state ***
...
# room names and types, to display on tiles    
# a room name will be repeated for multiple relsens in that room
...
def list_all_group_names():
    rels = db.session.query(Relsen.group_name).distinct()
    retval = [] 
    for r in rels: 
        retval.append (r.group_name)  
...
# Get devices from the database that are not enabled :
# Get the relsen tree of newly onboarded devices, which are currently disabled and waiting 
#    to be configured (& get activated in that process)    
@app.route('/get/new/devices', methods=['GET'])      
def get_new_devices():
    devs = Device.query.filter_by(enabled=False).all()
    retval = {}
    for dev in devs:
        retval.update (build_device_relsen_tree (dev))
    return retval    
...
def get_relsen_ids_by_name():    
    ...
    if (not relname):
        return ({'error' : 'relsen_name is required'})
    room = request.args.get('room_name')
    if (not room):
        return ({'error' : 'room_name is required'})
...
# onboard a new devices   
@app.route('/onboard/device', methods =['GET', 'POST']) 
def onboard_device_route():    
    if (request.method=='GET'):
        return ({'error':'POST the new Device id and Relsen list as JSON'})
    if (not request.json):
        return ({'error':'invalid Device data'})
...
from intof.Bridge import build_device_inventory, update_timer
